...
- 生成某个接口的测试前，必须按完整 `UniqueSymbol`、类名和方法名搜索目标测试仓库，并阅读所有对应已有测试。
- 若存在对应测试，必须先从 `.gme-agent/module_test_profile.md` 核对每条测试的输入和断言，只生成尚未覆盖的行为。
...
- 构建成功后，必须使用 `.gme-agent/generated_tests.json` 中的准确 filter 实际运行本次所有生成测试；仅构建通过不算完成。若测试异常退出或没有产生标准 GTest 结果，先分析并修正无效测试，再重新构建和运行。
...
构建通过后，必须用 `.gme-agent/generated_tests.json` 里的精确 filter 运行本次所有新增测试：
